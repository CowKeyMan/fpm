...
//
template <typename B, typename I, unsigned int F, bool R>
constexpr inline fixed<B, I, F, R> ceil(fixed<B, I, F, R> x) noexcept
{
...
template <typename B, typename I, unsigned int F, bool R>
constexpr inline fixed<B, I, F, R> floor(fixed<B, I, F, R> x) noexcept
{
    constexpr auto FRAC = B(1) << F;
    auto value = x.raw_value();
    if (value < 0) value -= FRAC - 1;
...
template <typename B, typename I, unsigned int F, bool R>
constexpr inline fixed<B, I, F, R> trunc(fixed<B, I, F, R> x) noexcept
{
    constexpr auto FRAC = B(1) << F;
...
template <typename B, typename I, unsigned int F, bool R>
constexpr inline fixed<B, I, F, R> round(fixed<B, I, F, R> x) noexcept
{
    constexpr auto FRAC = B(1) << F;
...
template <typename B, typename I, unsigned int F, bool R>
constexpr inline fixed<B, I, F, R> remquo(fixed<B, I, F, R> x, fixed<B, I, F, R> y, int* quo) noexcept
{
    assert(y.raw_value() != 0);
    assert(quo != nullptr);
...
template <typename B, typename I, unsigned int F, bool R>
constexpr inline fixed<B, I, F, R> modf(fixed<B, I, F, R> x, fixed<B, I, F, R>* iptr) noexcept
{
...
template <typename B, typename I, unsigned int F, bool R>
constexpr inline fixed<B, I, F, R> cos(fixed<B, I, F, R> x) noexcept
{
    using Fixed = fixed<B, I, F, R>;
...
template <typename B, typename I, unsigned int F, bool R>
constexpr void sincos(fixed<B, I, F, R> x, fixed<B, I, F, R>& out_sin, fixed<B, I, F, R>& out_cos) noexcept
{
    using Fixed = fixed<B, I, F, R>;

    // 1. Map x from [0..2*PI] domain into [0..4] domain
    x = fmod(x, Fixed::two_pi());
    x = x / Fixed::half_pi();

    // 2. Take x modulo one rotation, mapping to [0..4)
    if (x < Fixed(0)) {
        x += Fixed(4);
    }

    // 3. Compute sine components
    fixed<B, I, F, R> x_sin = x;
    int sign_sin = +1;
    if (x_sin > Fixed(2)) {
        sign_sin = -1;
        x_sin -= Fixed(2);
    ...
    const Fixed x2_sin = x_sin * x_sin;
    out_sin = sign_sin * x_sin * (Fixed::pi() - x2_sin * (Fixed::two_pi() - 5 - x2_sin * (Fixed::pi() - 3))) / 2;

    // 4. Compute cosine components (shifted by +1 in the [0..4] domain)
    fixed<B, I, F, R> x_cos = x + Fixed(1);
    if (x_cos >= Fixed(4)) {
        x_cos -= Fixed(4);
    }
    ...
    const Fixed x2_cos = x_cos * x_cos;
    out_cos = sign_cos * x_cos * (Fixed::pi() - x2_cos * (Fixed::two_pi() - 5 - x2_cos * (Fixed::pi() - 3))) / 2;
}
